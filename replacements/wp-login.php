...
?>
	<link  rel='stylesheet' type="text/css" media="all" href='/wp-content/plugins/spolecznosci/css/wp-login-3.css' />
...
<body class="login">
	<div id="index-head">
		<a id="logo" href="http://Miniblog.pl/"></a>
	</div>
	<div id="login">
...
<div style="margin:50px auto 0;width:900px;height:50px;">
<iframe id="news" src="http://miniblog.pl/news" style="border:0 none;width:900px;height:50px;overflow:hidden;"></iframe>
</div>
...
function retrieve_password() {
	...
	if ( empty( $_POST['user_login'] ) && !strpos($_POST['user_login'], '@') )
		$errors->add('empty_username', '<strong>BŁĄD</strong>: Podaj adres e-mail.');
	...
	header('Location: http://miniblog.pl/resetpass/'.$key.'/'.rawurlencode($user_login).'/'.rawurlencode($user_email));
	exit();
	return true;
}
...
case 'retrievepassword' :
	...
	login_header(__('Lost Password'), '<p class="message">' . 'Podaj adres email Twojego konta. Otrzymasz na ten adres link do utworzenia nowego hasła.' . '</p>', $errors);
...
<?php do_action('lostpassword_form'); ?>
	...
	<p class="submit"><input type="submit" name="wp-submit" id="wp-submit" class="button-primary" value="Wyślij" tabindex="100" /></p>
</form>
...
<form name="resetpassform" id="resetpassform" action="<?php echo site_url('wp-login.php?action=resetpass&key=' . urlencode($_GET['key']) . '&login=' . urlencode($_GET['login']), 'login_post') ?>" method="post">
	...
	<br class="clear" />
	<p class="submit"><input type="submit" name="wp-submit" id="wp-submit" class="button-primary" value="Zapisz" tabindex="100" /></p>
</form>
...
default:
	...
	if ( !empty($_POST['log']) ) {
		...
		if (!empty($user)) {//get_userdatabylogin
